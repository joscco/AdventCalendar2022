...
describe('Grid Position Calculator Tests', () => {
    test("Constructor with rows and columns work.", () => {
        expect(new Grid(2, 2)).not.toBeNull()
    })

    test("getPositionForIndex returns positions for valid indices.", () => {
        let gridCalculator = new Grid(2,3);
        expect(() => gridCalculator.getGlobalPositionForIndex({row: 0, column: 3}))
            .toThrowError()
        expect(() => gridCalculator.getGlobalPositionForIndex({row: 1, column: 2}))
            .not.toThrowError()
    })

    test("Position x and y defines anchor of index field (0, 0).", () => {
        let gridCalculator = new Grid(2,3);
        expect(gridCalculator.getGlobalPositionForIndex({row: 0, column: 0}))
    ...
    test("Setting tile width, height adapts width, height and position of tiles.", () => {
        ...
        expect(gridCalculator.getGlobalPositionForIndex({row: 0, column: 0})).toStrictEqual({x: 0, y: 0})
        expect(gridCalculator.getGlobalPositionForIndex({row: 1, column: 0})).toStrictEqual({x: 0, y: 100})
        expect(gridCalculator.getGlobalPositionForIndex({row: 0, column: 1})).toStrictEqual({x: 100, y: 0})
    })
